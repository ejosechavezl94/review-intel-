module.exports = async (req, res) => {
  ...
  if (contentLength && parseInt(contentLength, 10) > 100000) { // Max 100KB
  ...
  if (!competitors || !Array.isArray(competitors) || competitors.length === 0) {
    return res.status(400).json({ error: 'No hay datos de competidores válidos' });
  }
  if (competitors.length > 3) {
    return res.status(400).json({ error: 'Máximo 3 competidores permitidos' });
  }
...
PRODUCTO A LANZAR:
════════════════════════════════════════
${my_product ? String(my_product).substring(0, 1000).replace(/[<>]/g, '') : 'No especificado — analiza el mercado de forma genérica'}
